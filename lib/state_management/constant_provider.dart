...
class ConstantProvider extends ChangeNotifier{
  ...
  dynamic apiData = {};
  dynamic pumpList = [];
  dynamic agitatorList = [];
  dynamic selectorList = [];
  dynamic tankFloatList = [];
  List<String> lowerTankLevelList = [];
  List<String> upperTankLevelList = [];
  ...
  void fetchSettings(dynamic data){
    ...
    print("setting['levelSensor'] :: ${setting['levelSensor']}");
    setting['normalAlarm'] = {};
  ...
  void fetchAll(dynamic data){
    ...
    alarmType = [];
    autoIncrement = 0;
    print("levelSensorUpdated1 : ${levelSensorUpdated}");
    ...
    for(var i in data.entries){
      ...
      if(i.key == 'irrigationPump'){
        pumpList = [for(var i in i.value) i['id']];
      }
      if(i.key == 'agitator'){
        agitatorList = [for(var i in i.value) i['hid']];
      }
      if(i.key == 'selector'){
      ...
      if(i.key == 'default'){
        for(var j in i.value.entries){
          ...
          if(j.key == 'alarm'){
          ...
          else if(j.key == 'levelSensor'){
            //TODO: generating levelSensor
            for(var ls in j.value){
              levelSensorUpdated.add({
                'sNo' : ls['sNo'],
                'id' : ls['id'],
                'hid' : ls['hid'],
            ...
            print('levelSensorUpdated3 : ${levelSensorUpdated}');
            ...
            for(var fert in j.value){
              ...
              fertilizerUpdated.add({
                ...
                'agitator' : setting['fertilization']?['${fert['sNo']}']?['agitator'] ?? (agitatorList.isEmpty ? '' : agitatorList[0]),
                'selector' : setting['fertilization']?['${fert['sNo']}']?['selector'] ?? (selectorList.isEmpty ? '' : selectorList[0]),
          ...
          //TODO: generating pump
            ...
            for(var p in j.value){
              pumpUpdated.add({
                ...
                'lowerTankLevel' : setting['pump']?['${p['sNo']}']?['lowerTankLevel'] ?? '-',
                'upperTankLevel' : setting['pump']?['${p['sNo']}']?['upperTankLevel'] ?? '-',
              });
            }
          ...
          // else if(j.key == 'lowerTankLevelSensor'){
          //   lowerTankLevelList = [for(var i in j.value) i['hid']];
          // }
          // else if(j.key == 'upperTankLevelSensor'){
          //   upperTankLevelList = [for(var i in j.value) i['hid']];
          // }
        }
      }
    }
  ...
  dynamic sendDataToHW(){
    ...
    for(var i = 0;i < alarmUpdated.length;i++){
      ...
      for(var j = 0;j < type.length;j++){
        ...
        nAlarm += '${nAlarm.isNotEmpty ? ';' : ''}'
            ...
            '${type[j]['resetAfterIrrigation'] == 'Yes' ? (type[j]['alarmOnStatus'] == 'Stop Irrigation' ? 0 : 1) : 0},${type[j]['autoResetDuration']},${type[j]['threshold'] == '' ? '0' : type[j]['threshold']}' ;

        cAlarm += '${cAlarm.isNotEmpty ? ';' : ''}'
            '${i+1}.${j+1},'
            '${criticalAlarmUpdated[i]['hid']},${j+1},'
            '${typeC[j]['scanTime']},${alarmBehavior(typeC[j]['alarmOnStatus'])},'
            '${typeC[j]['resetAfterIrrigation'] == 'Yes' ? (typeC[j]['alarmOnStatus'] == 'Stop Irrigation' ? 0 : 1) : 0},${typeC[j]['autoResetDuration']},${typeC[j]['threshold'] == '' ? '0' : typeC[j]['threshold']}' ;
      }
    }
    ...
    for(var i in pumpUpdated){
      ...
      pump += '${pump.isNotEmpty ? ';' : ''}'
          ...
          '${i['topTankFeet'] == '' ? 0 : i['topTankFeet']},'
          '$useTopTankHigh,'
          '$useTopTankLow,'
          '$useSumpTankHigh,'
          '$useSumpTankLow,'
          '${hyphenToEmptyString(i['topTankHigh'])},'
          '${hyphenToEmptyString(i['topTankLow'])},'
          '${hyphenToEmptyString(i['sumpTankHigh'])},'
          '${hyphenToEmptyString(i['sumpTankLow'])},'
          '${pumpLevelHwPayload(i['upperTankLevel'])},'
          '${pumpLevelHwPayload(i['lowerTankLevel'])}'
      ;
    }
  ...
  String hyphenToEmptyString(value){
    if(value == '-'){
      return '';
    }else{
      if(value.contains(' - ')){
  ...
  String pumpLevelHwPayload(value){
    if(value == '-'){
      return '';
