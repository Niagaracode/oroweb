...
class HourlyData extends StatefulWidget {
  ...
  const HourlyData({super.key, required this.userId, required this.controllerId, this.nodeControllerId = 0, this.showAppBar = false});
...
class _HourlyDataState extends State<HourlyData> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: widget.showAppBar ? AppBar(title: const Text("Power and Motor"),)  :PreferredSize(preferredSize: Size.zero, child: Container()),
      body: SafeArea(
        child: LayoutBuilder(
          ...
          Column(
            children: [
              buildAnimatedContainer(
                  ...
                  highestValue: selectedIndex == 0 ? const Duration(hours: 30): parseTime(motorDataList[index].totalPowerOnTime)
              ),
              const SizedBox(height: 10,)
            ],
          )
