...
class ListOfLogConfig extends StatefulWidget {
  ...
  const ListOfLogConfig({super.key, required this.userId, required this.controllerId});
  @override
...
class _ListOfLogConfigState extends State<ListOfLogConfig> {
  Map<String,dynamic> irrigationLogParameterFromServer = {
    ...
    'centralEcPh' : {
      ...
      'CentralPhAverage' : ['Central Ph Avg',true],
      'CentralPhMaximum' : ['Central Ph Max',true],
      'CentralPhMinimum' : ['Central Ph Min',true],
      'CentralEcSetValue' : ['Central Ec Avg',true],
      'CentralEcAverage' : ['Central Ec Avg',true],
      'CentralEcMaximum' : ['Central Ec Max',true],
      'CentralEcMinimum' : ['Central Ec Min',true],
      'overAll' : ['over all',true]
    },
    '<C - CH1>' : {
  ...
  void getUserLogConfig()async{
    ...
    var overAllPvd = Provider.of<OverAllUse>(context,listen: false);
    try{
      print('getUserLogConfig => ${widget.userId}  ${widget.controllerId}');
      HttpService service = HttpService();
      var response = await service.postRequest(
          'getUserLogConfig',
          {
                                    ...
                                    IconButton(
                                        ...
                                        onPressed: ()async{
                                          setState(() {
                                            ...
                                            setOriginalToDuplicateParameter(originalParameterList: selectedIrrigationParameterArray.filterParameterList, duplicateParameterList: irrigationParameterArrayDuplicate.filterParameterList);
                                    ...
                                    IconButton(
                                        ...
                                        onPressed: () {
                                          Navigator.push(context, MaterialPageRoute(builder: (context){
                                            return LogHome(serverData: serverData['logConfig'][i], userId: widget.userId,controllerId: widget.controllerId,);
                                          }));
                                        },
          ...
          return Column(
            children: [
              SizedBox(
                ...
                height: 60,
                child: ListTile(
                  title: Text('<${central ? 'C' : 'L'} - CH$channelNo> Parameter',style: TextStyle(fontWeight: FontWeight.bold),),
                  leading: Checkbox(
        ...
        return Align(
          ...
          child: Material(
            ...
            child: StatefulBuilder(
              builder: (BuildContext context, StateSetter stateSetter) {
                return SizedBox(
                  width: constraints.maxWidth < 300 ? 280 : constraints.maxWidth * 0.6,
                  child: Scaffold(
