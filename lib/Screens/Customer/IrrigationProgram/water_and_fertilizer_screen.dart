...
class _WaterAndFertilizerScreenState extends State<WaterAndFertilizerScreen> {
  ...
  void _showTimePicker(IrrigationProgramProvider programPvd,OverAllUse overAllPvd,int index,String purpose,value,validation,[int? fertIndex]) async {
    // // print('showed');
    var fertilizerList = [
      if(programPvd.sequenceData[programPvd.selectedGroup]['centralDosing'].isNotEmpty)
  ...
  Widget waterSetting(int index,IrrigationProgramProvider programPvd,OverAllUse overAllPvd){
    if(index == 0){
      return ListTile(
        title: const Text('Method',style: TextStyle(color: Colors.black,fontSize: 14),),
        leading: SizedBox(
  ...
  Widget fertilizerSetting(int index,IrrigationProgramProvider programPvd,OverAllUse overAllPvd){
    if(index == 0){
      return ListTile(
...
Widget sequenceWidget({required bool top,required bool bottom,required int seq,required IrrigationProgramProvider programPvd}){
  return  SizedBox(
    width: 100,
    height: 50,
    child: Stack(
...
Widget currentSequenceDetails(IrrigationProgramProvider programPvd){
  return  Expanded(
    child: SizedBox(
      width: double.infinity,
      height: 50,
...
double returnWidth(IrrigationProgramProvider programPvd,String preOrPostValue,double screenWidth){
...
double returnWidthForProMethod(IrrigationProgramProvider programPvd,double screenWidth){
  var water = programPvd.waterValueInSec();
...
double returnWidthForFertilizer(IrrigationProgramProvider programPvd,dynamic fertilizer,double screenWidth){
  var water = programPvd.waterValueInSec();
...
List<dynamic> returnSelectedSiteRecipe(IrrigationProgramProvider programPvd){
  var list = [];
...
Widget mySeqButtons({required String name,required void Function() onTap,double? radius,double? verticalPadding,int? index,required IrrigationProgramProvider programPvd}){
  return InkWell(
    onTap: onTap,
    child: Container(
